...
for(let i = 0; i < boxes.length; i++){
    boxes[i].addEventListener("click", function(){
        ...
        let vez = checkElement(player1, player2);
...
function checkWinCondition() {
    ...
    if(b3.childNodes.length > 0 && b6.childNodes.length > 0 && b9.childNodes.length > 0){
        let b3Child = b9.childNodes[0].className;
        let b5Child = b6.childNodes[0].className;
        let b9Child = b3.childNodes[0].className;

        if(b3Child == 'x' && b5Child == 'x' && b9Child === 'x'){
            declareWinner('x');
        }
        if(b3Child == 'o' && b5Child == 'o' && b9Child === 'o'){
            declareWinner('o');
        }
    }
